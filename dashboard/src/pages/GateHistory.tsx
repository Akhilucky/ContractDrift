...
import { getGateHistory, getDeployments } from '../lib/api'
import DeploymentTimeline from '../components/DeploymentTimeline'
import type { DeploymentEvent } from '../components/DeploymentTimeline'
...
export default function GateHistory() {
  ...
  const timelineEvents: DeploymentEvent[] = useMemo(() => {
    if (!deployments) {
      return filtered.map((g) => ({
        id: g.id,
        serviceId: g.service,
        versionSha: g.version,
        environment: g.environment,
        deployedAt: g.created_at,
        status: g.decision === 'DENY' ? 'blocked' as const : g.decision === 'OVERRIDE' ? 'overridden' as const : 'success' as const,
        violations: [],
      }))
    }
    let devents = deployments
    if (serviceFilter) {
      devents = devents.filter((d) => d.service_id === serviceFilter)
    }
    return devents.map((d) => ({
      id: d.id,
      serviceId: d.service_id,
      versionSha: d.version_sha,
      environment: d.environment,
      deployedAt: d.deployed_at,
      status: d.status as 'success' | 'blocked' | 'overridden',
      violations: d.violations?.map((v) => ({ severity: v.severity, description: v.message })),
    }))
  }, [deployments, filtered, serviceFilter])
        ...
        <div className="flex items-center gap-3">
          <div className="flex bg-slate-800 border border-slate-700 rounded-lg overflow-hidden">
            ...
            <button
              ...
              className={`px-3 py-1.5 text-sm font-medium transition-colors ${
                view === 'timeline' ? 'bg-slate-700 text-white' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              Timeline
            </button>
